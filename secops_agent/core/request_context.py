...
def _technical_goal(text: str) -> TechnicalGoal:
    if _contains_any(
        ...
        (
            ...
            "kernel",
            "uname",
            # E3: current user / uptime / active interface / default gateway are
            # local facts — answer them deterministically instead of routing to
            # the flaky model (both EN and FR phrasings, accent-stripped).
            "current user",
            "whoami",
            "who am i",
            "logged in as",
